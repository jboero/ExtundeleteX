...
std::ostream &operator<<(std::ostream &os, const ext2_inode &inode) {
  ...
  os << "Double indirect block: " << inode.i_block[EXT2_DIND_BLOCK]
     << std::endl;
  os << "Triple indirect block: " << inode.i_block[EXT2_TIND_BLOCK]
     << std::endl;
...
std::ostream &
operator<<(std::ostream &os, const ext2_super_block *const s_block) {
  // This was largely generated with:
  ...
  os << "# Fragments per group: " << EXT2_FRAGS_PER_BLOCK(s_block)
     << std::endl;
  ...
  os << "Default uid for reserved blocks: " << s_block->s_def_resuid
     << std::endl;
  os << "Default gid for reserved blocks: " << s_block->s_def_resgid
     << std::endl;
  os << "First non-reserved inode: " << s_block->s_first_ino << std::endl;
  os << "size of inode structure: " << s_block->s_inode_size << std::endl;
  os << "block group # of this superblock: " << s_block->s_block_group_nr
     << std::endl;
  os << "compatible feature set: " << s_block->s_feature_compat << std::endl;
  os << "incompatible feature set: " << s_block->s_feature_incompat
     << std::endl;
  os << "readonly-compatible feature set: " << s_block->s_feature_ro_compat
     << std::endl;
  ...
  os << "Nr to preallocate for dirs: " << (int) s_block->s_prealloc_dir_blocks
     << std::endl;
  os << "Per group table for online growth: " << s_block->s_reserved_gdt_blocks
     << std::endl;
  ...
  os << "inode number of journal file: " << s_block->s_journal_inum
     << std::endl;
  os << "device number of journal file: " << s_block->s_journal_dev
     << std::endl;
  os << "start of list of inodes to delete: " << s_block->s_last_orphan
     << std::endl;
  os << "HTREE hash seed: ";
  for (int n = 0; n < 4; n++)
  ...
  os << "Default hash version to use: " << (int) s_block->s_def_hash_version
     << std::endl;
  os << "Default type of journal backup: " << (int) s_block->s_jnl_backup_type
     << std::endl;
  os << "First metablock group: " << s_block->s_first_meta_bg << std::endl;
  os << "When the filesystem was created: " << s_block->s_mkfs_time
     << std::endl;
...
std::ostream &operator<<(std::ostream &os, const ext2_group_desc &group_desc) {
  os << "block bitmap at " << group_desc.bg_block_bitmap
     ...
     << std::endl;
  os << "\t   "
     << group_desc.bg_free_blocks_count << " free blocks, "
...
std::ostream &
operator<<(std::ostream &os, journal_header_t const &journal_header) {
  os << "Block type: ";
...
std::ostream &operator<<(std::ostream &os,
                         const journal_revoke_header_t journal_revoke_header) {
  ...
  count /= sizeof(__u32);
  __u32 const *ptr = reinterpret_cast<__u32 const *>(
    (unsigned char const *) &journal_revoke_header +
...
std::ostream &
operator<<(std::ostream &os, journal_superblock_t const &journal_super_block) {
  os << "Journal Super Block:" << std::endl;
  os << "Signature: 0x" << tohex(journal_super_block.s_header.h_magic, 8)
     << std::endl;
  os << journal_super_block.s_header << std::endl;
  os << "Journal block size: " << journal_super_block.s_blocksize << std::endl;
  os << "Number of journal blocks: " << journal_super_block.s_maxlen
     << std::endl;
  os << "Journal block where the journal actually starts: "
     << journal_super_block.s_first << std::endl;
  os << "Sequence number of first transaction: "
     << (journal_super_block.s_sequence) << std::endl;
  os << "Journal block of first transaction: " << (journal_super_block.s_start)
     << std::endl;
  os << "Error number: " << (journal_super_block.s_errno) << std::endl;
  if ((journal_super_block.s_header.h_blocktype) != JFS_SUPERBLOCK_V2)
    return os;
  os << "Compatible Features: " << (journal_super_block.s_feature_compat)
     << std::endl;
  os << "Incompatible features: " << (journal_super_block.s_feature_incompat)
     << std::endl;
  os << "Read only compatible features: "
     << (journal_super_block.s_feature_ro_compat) << std::endl;
  os << "Journal UUID: 0x";
  for (int i = 0; i < 16; ++i)
    os << std::hex << std::setfill('0') << std::setw(2)
       << (int) (journal_super_block.s_uuid[i]);
  os << std::dec << std::endl;
  int32_t nr_users = (journal_super_block.s_nr_users);
  os << "Number of file systems using journal: " << nr_users << std::endl;
  assert(nr_users <= 48);
  os << "Location of superblock copy: " << (journal_super_block.s_dynsuper)
     << std::endl;
  os << "Max journal blocks per transaction: "
     << (journal_super_block.s_max_transaction) << std::endl;
  os << "Max file system blocks per transaction: "
     << (journal_super_block.s_max_trans_data) << std::endl;
  os << "IDs of all file systems using the journal:\n";
  for (int u = 0; u < nr_users; ++u) {
    os << (u + 1) << '.';
    for (int i = 0; i < 16; ++i)
      os << std::hex << " 0x" << std::setfill('0') << std::setw(2)
         << (int) (journal_super_block.s_users[u * 16 + i]);
    os << std::dec << std::endl;
  }
  return os;
}
