...
static void
Log_errors(const char *whoami, long code, const char *format, va_list args) {
  char buffer[BUFSIZ];
  vsprintf(buffer, format, args);
  Log::error << whoami << ": " << error_message(code) << " " << buffer
             << std::endl;
}
...
static errcode_t examine_fs(ext2_filsys fs) {
  ...
  if (errcode) {
    com_err(Config::progname.c_str(), errcode,
            "while reading journal superblock.");
    return errcode;
  }
  ...
  if (!Config::restore_file.empty() ||
    ...
    if (errcode) {
      com_err(Config::progname.c_str(), errcode,
              "while reading journal descriptors.");
      return errcode;
    }
...
static int decode_options(int &argc, char **&argv) {
  ...
  while (
    (short_option = getopt_long(argc, argv, "j:vVb:B:o:", longopts, NULL)) !=
    -1) {
    switch (short_option) {
      case 0:
        switch (long_option) {
          ...
          case opt_after:
            ...
            if (errno) {
              Log::error << "Invalid parameter: --after " << optarg
                         << std::endl;
          ...
          case opt_before:
            ...
            if (errno) {
              Log::error << "Invalid parameter: --before " << optarg
                         << std::endl;
              return EU_DECODE_FAIL;
            }
            break;
          ...
          case opt_inode_to_block:
            ...
            if (errno) {
              Log::error << "Invalid parameter: --inode-to-block " << optarg
                         << std::endl;
              return EU_DECODE_FAIL;
            }
            if (Config::inode_to_block < 1) {
              Log::error << Config::progname << ": --inode-to-block: inode "
                         << Config::inode_to_block << " is out of range."
                         << std::endl;
              return EU_DECODE_FAIL;
            }
            break;
          case opt_inode:
            ...
            if (errno) {
              Log::error << "Invalid parameter: --inode " << optarg
                         << std::endl;
              return EU_DECODE_FAIL;
            }
            if (Config::inode < 1) {
              Log::error << Config::progname << ": --inode: inode "
                         << Config::inode
                         << " is out of range." << std::endl;
              return EU_DECODE_FAIL;
            }
          ...
          case opt_block:
            ...
            if (errno) {
              Log::error << "Invalid parameter: --block " << optarg
                         << std::endl;
              return EU_DECODE_FAIL;
            }
            if (Config::block < 1) {
              Log::error << Config::progname << ": --block: block "
                         << Config::block
                         << " is out of range." << std::endl;
              return EU_DECODE_FAIL;
            }
          ...
          case opt_show_journal_inodes:
            ...
            if (errno) {
              Log::error << "Invalid parameter: --show-journal-inodes "
                         << optarg << std::endl;
              return EU_DECODE_FAIL;
            }
            if (Config::show_journal_inodes < 1) {
              Log::error << Config::progname
                         << ": --show-journal-inodes: inode "
          ...
          case opt_journal_transaction:
            ...
            if (errno) {
              Log::error << "Invalid parameter: --journal-transaction "
                         << optarg << std::endl;
              return EU_DECODE_FAIL;
            }
            break;
  ...
  if (commandline_before < LONG_MAX || commandline_after) {
    Log::status
      << "Only show and process deleted entries if they are deleted ";
  ...
  if (argc > 1) {
    Log::error << Config::progname
               << ": Some unrecognized options were found. "
...
static errcode_t init_fs(const char *fsname, ext2_filsys *ret_fs) {
  ...
  if (error == 0) {
    if (S_ISDIR (statbuf.st_mode)) {
      Log::error << Config::progname << ": \"" << fsname
                 << "\" is a directory. You need "
                 << "to use the raw filesystem device (or a copy thereof)."
                 << std::endl;
      return EU_FS_ERR;
    }
    if (!S_ISBLK(statbuf.st_mode) && statbuf.st_size < 2048) {
      Log::error << Config::progname << ": \"" << fsname
                 ...
                 << " filesystem (" << statbuf.st_size << " bytes)."
                 << std::endl;
      return EU_FS_ERR;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (errcode) {
    ...
    Log::error << Config::progname << ": Error parsing command-line options."
               << std::endl;
    return errcode;
  }
  ...
  if (errcode) {
    std::cout << std::flush;
    com_err(Config::progname.c_str(), errcode,
            "when trying to open filesystem %s", *argv);
    return errcode;
  }
  ...
  if (errcode == EU_FS_RECOVER) {
    ...
    com_err(Config::progname.c_str(), errcode,
            "when trying to load filesystem parameters");
    return errcode;
  }
  ...
  if (errcode) {
    com_err(Config::progname.c_str(), errcode,
            "when trying to examine filesystem");
    return errcode;
  }
