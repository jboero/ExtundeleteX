...
std::ostream Log::info(nullptr);
std::ostream Log::debug(nullptr);
...
struct triad {
  ...
  triad(const T1 &x, const T2 &y, const T3 &z) : first(x), second(y),
                                                 third(z) {}

  template<class U, class V, class W>
  triad(const triad <U, V, W> &p) : first(p.first), second(p.second),
                                    third(p.third) {}
};
...
static int
extundelete_test_inode_bitmap(const ext2_filsys fs, ext2_ino_t ino) {
  int allocated = -1;
...
*/
static int
extundelete_test_block_bitmap(ext2fs_block_bitmap block_map, blk64_t blocknr) {
  int allocated;
...
/*static*/ ext2_ino_t block_to_inode(const ext2_filsys fs, blk_t block) {
  ...
  if (block >= inode_table &&
      (size_t) EXT2_BLOCK_SIZE(fs->super) * (block + 1) <=
      (size_t) EXT2_BLOCK_SIZE(fs->super) * inode_table +
      EXT2_INODES_PER_GROUP(fs->super) * EXT2_INODE_SIZE(fs->super)) {
    return (ext2_ino_t) (1 + group * EXT2_INODES_PER_GROUP(fs->super)
                         + (size_t) EXT2_BLOCK_SIZE(fs->super)
                         * (block - inode_table) / EXT2_INODE_SIZE(fs->super));
  } else {
    return 0;
  }
}
...
int load_super_block(ext2_filsys fs) {
  ...
  if (groups_ * inodes_per_group_ != fs->super->s_inodes_count)
    return EU_FS_ERR;
  // The inode bitmap has to fit in a single block.
  if (inodes_per_group_ > (unsigned) 8 * EXT2_BLOCK_SIZE(fs->super))
    return EU_FS_ERR;
  // Each inode must fit within one block.
  if (EXT2_INODE_SIZE(fs->super) > EXT2_BLOCK_SIZE(fs->super))
    return EU_FS_ERR;
  // inode_size must be a power of 2.
  if ((EXT2_INODE_SIZE(fs->super) - 1) & EXT2_INODE_SIZE(fs->super))
    return EU_FS_ERR;
  // There should fit exactly an integer number of inodes in one block.
  if ((EXT2_BLOCK_SIZE(fs->super) / EXT2_INODE_SIZE(fs->super)) *
  ...
  if ((super_block.s_feature_compat & EXT2_FEATURE_COMPAT_IMAGIC_INODES))
    Log::warn
      << "WARNING: Unknown file system feature: EXT2_FEATURE_COMPAT_IMAGIC_INODES\n";
  ...
  if ((super_block.s_feature_incompat & EXT2_FEATURE_INCOMPAT_COMPRESSION))
    Log::warn
      << "WARNING: File systems with EXT2_FEATURE_INCOMPAT_COMPRESSION set (like this one) have not been tested.\n";
  ...
  if ((super_block.s_feature_incompat & EXT3_FEATURE_INCOMPAT_JOURNAL_DEV))
    Log::warn
      << "WARNING: extundelete may have problems reading from an external journal.\n";
...
void print_directory_inode(ext2_filsys fs, struct ext2_inode *inode,
  ...
  struct dir_context ctx = {0,
                            DIRENT_FLAG_INCLUDE_REMOVED, buf, print_entry, fs,
                            0};
...
errcode_t read_block64(ext2_filsys fs, blk64_t *blocknr, e2_blkcnt_t blockcnt,
  ...
  retval = io_channel_read_blk(fs->io, (blk_t) *blocknr, 1,
                               ...
                               EXT2_BLOCK_SIZE(fs->super) * blockcnt);
#endif
  return retval;
}
...
void classify_block(ext2_filsys fs, blk64_t blocknr) {
  ...
  std::cout << "Block " << blocknr << " is in group "
            << ext2fs_group_of_blk2(fs, blocknr)
            << "." << std::endl;
  ...
  if (retval == 0) {
    ...
    std::cout << "| Inode number | Deleted status\n";
    struct dir_context ctx = {0,
                              DIRENT_FLAG_INCLUDE_REMOVED, block, print_entry,
                              fs, 0};
...
static int
get_block_nums64(ext2_filsys /*fs*/, blk64_t *blocknr, e2_blkcnt_t blockcnt,
...
static int block_is_journal(ext2_filsys /*fs*/, blk64_t *blocknr,
...
/*static*/ bool is_journal(ext2_filsys fs, blk64_t block) {
  ...
  if (block == 0) {
    Log::error
      << "Invalid block number 0 encountered when finding blocks in the journal"
      << std::endl;
    return flag;
  }
  ...
  errcode = extundelete_block_iterate3(fs, *inode, 0, 0, block_is_journal,
                                       &block);
  if (errcode) {
...
static int entry_iterate(ext2_ino_t /*dir*/, int entry,
  ...
  if (dirent2->file_type == EXT2_FT_DIR && ps->ino != dirent2->inode) {
    ...
    it = std::find(ps->parent_inos.begin(), ps->parent_inos.end(),
                   dirent2->inode);
...
static int
process_deleted_dir(ext2_filsys fs, blk64_t *blocknr, e2_blkcnt_t /*blockcnt*/,
                    blk64_t /*ref*/, int /*off*/, void *priv) {
...
*/
static int pair_names_with(ext2_filsys fs, ext2_filsys jfs,
                           std::vector<ext2_ino_t> &inolist,
  ...
  if (retval == 0 && LINUX_S_ISDIR(inode->i_mode)) {
    ...
    struct dir_context ctx = {0,
                              DIRENT_FLAG_INCLUDE_REMOVED, buf, entry_iterate,
                              &ps, 0};
  ...
  if (ino == EXT2_ROOT_INO) {
    ...
    for (it = rvk_block.begin(); it != rvk_block.end(); it++) {
      blk64_t blknum = *it;
      struct pair_struct ps = {fs, jfs, inolist, ino, "lost+found", 1,
                               parent_inos};
      struct dir_context ctx = {0,
                                DIRENT_FLAG_INCLUDE_REMOVED, buf,
                                entry_iterate, &ps, 0};
...
int restore_directory(ext2_filsys fs, ext2_filsys jfs, ext2_ino_t dirino,
                      std::string dirname) {
  errcode_t retval = 0;
  ...
  for (it = tag_fsblk.rbegin(), jit = tag_jblk.rbegin(), sit = tag_seq.rbegin();
       ...
       it++, jit++, sit++) {
    blk64_t blknum = *it;
    std::map<blk64_t, int>::iterator bgit = block_to_group_map.lower_bound(
      blknum);
    ...
    if (blknum - (*bgit).first < max_block) {
      ...
      ext2_ino_t firstino = (ext2_ino_t) (
        (blknum - blknum1) * EXT2_BLOCK_SIZE(fs->super) /
        EXT2_INODE_SIZE(fs->super)
        + group * EXT2_INODES_PER_GROUP(fs->super) + 1);
      retval = read_journal_block(jfs, *jit, buf);
      if (retval) continue;
      for (ext2_ino_t ino = firstino; ino < firstino +
        ...
        if (inode->i_dtime > 0) {
          if ((dit = deleted_inodes_map.find(ino)) !=
              deleted_inodes_map.end()) {
            if ((*dit).second > *sit) {
              (*dit).second = *sit;
            }
          } else {
            deleted_inodes_map.insert(
              std::pair<ext2_ino_t, uint32_t>(ino, *sit));
          }
        }
  ...
  Log::status << recoverable_inodes.size() << " recoverable inodes found."
              << std::endl;
  ...
  pair_names_with(fs, jfs, recoverable_inodes, dirino, dirname, pnflag,
                  parent_inos);
  Log::status << recoverable_inodes.size() << " recoverable inodes still lost."
              << std::endl;
...
int init_journal(ext2_filsys fs, ext2_filsys jfs, journal_superblock_t *jsb) {
  ...
  journal_inode = (struct ext2_inode *) operator new(
    EXT2_INODE_SIZE(fs->super));
  ext2_ino_t journal_ino = fs->super->s_journal_inum;
  retval = ext2fs_read_inode_full(fs, journal_ino, journal_inode,
                                  EXT2_INODE_SIZE(fs->super));
  ...
  if (fs->super->s_journal_inum) {
    extundelete_block_iterate3(fs, *journal_inode, BLOCK_FLAG_DATA_ONLY, 0,
                               get_block_nums64, blocks);
  } else
      ...
      switch (descriptor->h_blocktype) {
        case JFS_DESCRIPTOR_BLOCK: {
          ...
          do {
            ...
            if (n > jsb->s_maxlen) {
              ...
              j_tags.push_back(triad<uint32_t, blk64_t, blk64_t>(seq, blocks[n], jbt->t_blocknr));
              Log::debug << std::endl << "Journalled block " << blocks[n]
                         << " is a copy of block " << jbt->t_blocknr;
            }
            flags = jbt->t_flags;
            if (!(flags & JFS_FLAG_SAME_UUID)) {
              jbt = reinterpret_cast<journal_block_tag_t *>((char *) jbt + sizeof(jsb->s_uuid));
            }
            jbt = reinterpret_cast<journal_block_tag_t *>((char *) jbt + journ_tag_bytes(jsb));
        ...
        case JFS_REVOKE_BLOCK: {
          ...
          for (int64_t count = init_count; count < rvk->r_count;
               count += skip) {
            blk64_t blk;
            if (JOURNAL_HAS_INCOMPAT_FEATURE(jsb,
...
static blk64_t
journ_get_dir_block(ext2_filsys /*fs*/, blk64_t blknum, void * /*buf*/) {
  if (blknum == 0)
    return 0;
...
static int
match_ino(ext2_filsys fs, blk64_t *blocknr, e2_blkcnt_t /*blockcnt*/,
          blk64_t /*ref*/, int /*off*/, void *priv) {
  errcode_t retval = 0;
...
static ext2_ino_t
find_inode(ext2_filsys fs, ext2_filsys jfs, struct ext2_inode *inode,
  ...
  *new_ino = 0;
  ...
  if (code != 0 && code != BLOCK_ABORT) {
    com_err("extundelete", code, "while finding inode for %s",
            curr_part.c_str());
    ino2 = 0;
  }
...
*/
errcode_t
restore_file(ext2_filsys fs, ext2_filsys jfs, const std::string &fname) {
  ...
  *new_ino = EXT2_ROOT_INO;
  ...
  if (ino2 == 0) {
    ...
    for (rit = rvk_block.rbegin(); rit != rvk_block.rend(); ++rit) {
      *new_ino = 0;
      ...
      priv->curr_name = curr_part;
      struct dir_context ctx = {0,
                                DIRENT_FLAG_INCLUDE_REMOVED, buf, match_name2,
                                priv, 0};
  ...
  Log::debug << "Post directory block search inode number: " << ino
             << std::endl;
  Log::debug << "Next file part: " << curr_part << std::endl;
  Log::debug << "File name and place: " << fname << "   " << place
             << std::endl;
  ...
  Log::debug << "Post-revoke search current inode number: "
             << ino
             << std::endl;
  Log::debug << "Next file part: " << curr_part << std::endl;
...
inode_is_valid(const ext2_filsys fs, const struct ext2_inode *const inode) {
...
static errcode_t recover_inode(ext2_filsys fs, ext2_filsys jfs, ext2_ino_t ino,
  ...
  blk64_t blknum1 = ext2fs_inode_table_loc(fs, group);
  blk64_t blknum2 = (ino - 1 - group * EXT2_INODES_PER_GROUP(fs->super)) *
                    EXT2_INODE_SIZE(fs->super) / EXT2_BLOCK_SIZE(fs->super);
  ...
  for (rit = oldblks2.rbegin(); rit != oldblks2.rend(); ++rit) {
    ...
    parse_inode_block(fs, inode, buf, ino);
    if (inode->i_dtime != 0 &&
        ((int64_t) (inode->i_dtime)) >= commandline_after &&
...
static int first_block_is_allocated(ext2_filsys fs, blk64_t *blocknr,
                                    e2_blkcnt_t /*blockcnt*/,
...
errcode_t restore_inode(ext2_filsys fs, ext2_filsys jfs, ext2_ino_t ino,
  ...
  retval = extundelete_block_iterate3(fs, *inode, BLOCK_FLAG_DATA_ONLY, NULL,
                                      first_block_is_allocated, &allocated);
  if (retval) {
    Log::warn << "Unable to restore inode " << ino << " (" << fname
  ...
  if (LINUX_S_ISREG(inode->i_mode)) {
    ...
    if (file.is_open()) {
      ...
      flag = extundelete_block_iterate3(fs, *inode, BLOCK_FLAG_DATA_ONLY, NULL,
                                        write_block, &bufstruct);
      file.seekg(0, std::ios::end);
      fsize = file.tellg();
      file.close();

      if (!flag) {
        std::streamoff rsize = EXT2_BLOCK_SIZE(fs->super)
                               - inode->i_size % EXT2_BLOCK_SIZE(fs->super);
        ...
        retval = rename((outputdir + fname2).c_str(),
                        (outputdir + fname2 + ".part").c_str());
        if (retval) {
          Log::warn << "extundelete: " << strerror(errno)
                    << " when renaming file "
    ...
    Log::info << "extundelete identified inode " << ino << " as "
              << (outputdir + fname2) << ":"
              << "The inode does not correspond to a regular file."
              << std::endl;
    retval = EU_RESTORE_FAIL;
  }
...
static void parse_inode_block(ext2_filsys fs, struct ext2_inode *inode,
  ...
  int offset =
    (ino - 1) % (EXT2_BLOCK_SIZE(fs->super) / EXT2_INODE_SIZE(fs->super));
...
int get_journal_fs(ext2_filsys fs, ext2_filsys *jfs,
  ...
  if (fs->super->s_journal_inum) {
    ...
    *jfs = fs;
    ...
    if (journal_filename.empty()) {
      Log::error << "Must specify the external journal device name"
                 << std::endl;
...
int read_journal_superblock(ext2_filsys fs, ext2_filsys jfs,
  ...
  if (fs->super->s_journal_inum) {
    ...
    inode = (struct ext2_inode *) operator new(EXT2_INODE_SIZE(fs->super));
    errcode = ext2fs_read_inode_full(fs, fs->super->s_journal_inum, inode,
                                     EXT2_INODE_SIZE(fs->super));
    if (errcode) {
    ...
    errcode = ext2fs_bmap2(fs, fs->super->s_journal_inum, inode, NULL, 0, 0,
                           NULL, &blknum);
    if (errcode) {
      com_err("extundelete", errcode,
              "while mapping internal journal superblock");
      retval = errcode;
      goto finally;
    }
    ...
    if (errcode) {
      com_err("extundelete", errcode,
              "while reading internal journal superblock");
      retval = errcode;
      goto finally;
    }
  } else {
    ...
    if ((errcode = read_block64(jfs, &blknum, 0, 0, 0, buf))) {
      com_err("extundelete", errcode,
              "while reading external journal superblock");
      retval = errcode;
      goto finally;
    }
  }
  ...
  if (journal_superblock->s_header.h_magic != JFS_MAGIC_NUMBER)
    retval = EU_FS_ERR;
...
int print_inode(ext2_filsys fs, ext2_ino_t ino) {
  ...
  dump_hex_to(std::cout, reinterpret_cast<char const *> (inode),
              EXT2_INODE_SIZE(fs->super));
  std::cout << std::endl;
